...
namespace ReportChecker.Checkers
{
	...
	{
		HashSet<string> validThumbinails;

		public DigitalSignatureChecker(IEnumerable<string> validThumbinails)
		{
			this.validThumbinails = new HashSet<string>(validThumbinails);
		}
		...
		public CheckResult Check(string file)
		{
			...
			using (var package = Package.Open(file))
			{
				...
				{
					if (!validThumbinails.Contains(signer))
					{
						result.Success = false;
